...
/* ★ **지금 싸우는** 캐릭터 ID. 쿄=0 … 가일=17. 선택화면=255.
   ⚠ 예전에는 `0x08A0`/`0x08C0` 을 썼는데 그건 **팀 명단의 머리**다 —
     팀 대전에서 캐릭터가 바뀌어도 안 변해서 **SP 배치가 기준 캐릭터로 굳었다**(유저 제보).
   구조(팀 대전 실측, 화면 HUD 가 증인):
       08A0  00 06 03 | 00 06 03   P1 명단[쿄,이오리,료] · 뒤 세 칸은 «현재부터 회전»
       08C0  00 06 01 | 06 01 00   P2 명단[쿄,이오리,테리] · 회전 → 머리가 이오리
     화면은 P2 를 이오리라 적고 있었다. 곧 **회전한 쪽의 머리(+3)** 가 현재다.
   ⚠ 단일 대전에서는 `08A0 == 08A3` 이라 무해하다(세이브 여섯·캐릭터 다섯으로 확인).
   ⚠ 메뉴에서도 안전하다 — 캐릭선택·순서선택에서 `08A3` 은 255 다(옛 자리는 순서선택에서
     0 이라 **오히려 더 헐렁했다**). `svc_in_battle` 문지기가 그대로 선다. */
#define OFF_CHAR1   0x08A3   /* P1 현재 캐릭터 (명단 머리는 0x08A0) */
#define OFF_CHAR2   0x08C3   /* P2 현재 캐릭터 (명단 머리는 0x08C0) */
