...
/* ── RAM 맵 (tools/svc 실측) ─────────────────────────────────────── */
/* ★ **지금 싸우는** 캐릭터 ID. 쿄=0 … 가일=17. 선택화면=255.
   세 번째 자리다. 앞의 둘은 틀렸고, 둘 다 «잰 것»이 아니라 «가정»이었다.
     ① `0x08A0` — 팀 명단의 머리. 팀 대전에서 안 변한다(SP 가 리더로 굳었다).
     ② `0x08A3` — P2 한 판 덤프에서 뒤 세 칸이 회전하는 것을 보고 「머리가 현재」로
        읽었다. **P1 은 회전하지 않았고**, 유저가 「리더가 첫 차례가 아니어도 리더
        기술이 나온다」고 확인했다. +3 은 «출전 순서»(경기 내내 고정)이지 현재가 아니다.
   ③ **실측** (2026-09-07, team_fight.st 를 6000프레임 가만 두어 CPU 에 두 번 KO,
      0x0880~0x08D0 을 120프레임마다 덤프 + 캐릭터 칸 매 프레임 관찰, tools/svc/team_char.py):
       08A0  00 06 03   명단(안 변함)          08A3  00 06 03   출전 순서(안 변함)
       08A6  00 → 06 → 03   ★ KO 마다 바뀐다  08A9  00 → 01 → 02   출전 순서의 색인
     화면 HUD 가 증인 — 교대 뒤 「IORI VS IORI」, 두 번째 교대 뒤 「MAI VS IORI」.
     곧 08A6 = 08A3[08A9]. P2 도 같은 꼴(08C6 = 08C3[08C9]).
   ⚠ 단일 대전에서는 `08A6 == 08A3 == 08A0`(세이브 전부·캐릭터 18 전수) — 무해하다.
   ⚠ 메뉴에서도 안전하다 — 캐릭선택에서 `08A6` 은 255, `svc_in_battle` 문지기가 선다.
     (팀 라운드 개시 상태 team_r1.st 에서는 08A3 이 255 인데 08A6 은 이미 0 이다 —
      +3 은 나중에 채워지는 사본이다.) */
#define OFF_CHAR1   0x08A6   /* P1 지금 싸우는 캐릭터 = 출전순서[색인] */
#define OFF_CHAR2   0x08C6   /* P2 */
#define OFF_TEAM1   0x08A0   /* P1 팀 명단 세 칸 — 판정에는 안 쓴다 */
#define OFF_TEAM2   0x08C0   /* P2 팀 명단 세 칸 */
#define OFF_ORDER1  0x08A3   /* P1 출전 순서 세 칸 (경기 내내 고정) — 판정에는 안 쓴다 */
#define OFF_ORDIDX1 0x08A9   /* P1 출전 순서 색인 0~2 */
