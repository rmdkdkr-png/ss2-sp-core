...
/* 좌우 반전 — **bit7** 이다(SVC 의 `0x092C` bit7 과 같은 꼴).
   ...
   ★ **P1 것만 믿어라. 「うごかない」 더미의 플래그는 갱신되지 않는다.**
     CPU 무대에서는 P1=0 / P2=129 로 서로 마주보는데, 정지 무대에서는 P2 가 1 로 굳어 있다
     (같은 프레임에 화면상 둘은 분명히 마주보고 있다). 그래서 「P1·P2 의 bit7 이 다르다」를
     전투 판별 불변식으로 쓰려던 것을 **폐기**했다 — 정지 무대에서 거짓이 된다.
     P1 은 늘 살아 있으므로 P1 의 플래그는 유효하다(교차 프레임에 0→128 로 정확히 뒤집혔다). */
