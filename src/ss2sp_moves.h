...
/* SP 슬롯 기본 배치. 순서: n f b d df db air, -1 = 없음
   ★ 이 표는 **사용자가 직접 짠 것**이다 (2026-08-17, 배치표 편집기 JSON 반영).
     자동 생성값이 아니다 — 겹쳐 둔 자리(겐주로 d·db 등)도 의도된 것이니 고치지 말 것.
   ★ 기술 배열은 자리 순서에 맞춰 재정렬돼 있다 — mv[0]이 항상 중립 자리다.
     그래서 버튼 직결의 SP1 과 방향 없이 누른 SP 가 같은 기술이 된다.
   ★ 210자리 중 157자리가 채워져 있고, 서로 다른 기술 150기(/171기)가 SP+방향에 걸린다.
     기본값에서 뺀 것은 카드 필요·실기 미검증 기술 — 배치 메뉴에서 직접 꽂으면 쓸 수 있다. */
