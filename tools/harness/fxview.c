...
int main(int argc,char**argv){
  ...
  {int c1=argc>2?atoi(argv[2]):2, c2=argc>3?atoi(argv[3]):8; ram[BLK1]=8*(2*c1+0); ram[BLK2]=8*(2*c2+0); ram[0x17DF]=c2; ram[0x17E3]=0;}
  ...
  snap(argv[1]);                              /* 평시 */
  hold(60,0xF1,8,128,20);
  ss2comm_side(fb, TW, SW, TH, 0);
  ss2comm_side(fb+SW+GW, TW, SW, TH, 1);
  snap(argv[4]);                              /* 빈사(상대) */
  hold(90,0xF1,8,128,0);
  ss2comm_side(fb, TW, SW, TH, 0);
  ss2comm_side(fb+SW+GW, TW, SW, TH, 1);
  snap(argv[5]);                              /* KO — 상대 흑백 */
  return 0;
}
