...
int main(int argc,char**argv){
  ...
  G_ram=ram; G_rlen=rlen; trace_init();   /* RETRO_MEMORY_SYSTEM_RAM = 2 */
  ...
  while(sf && fgets(line,sizeof line,sf)){
    ...
    if(*p=='!'){
      ...
      if(k>=2 && !strcmp(cmd,"tag")){ snprintf(TRC_TAG,sizeof TRC_TAG,"%s",arg); continue; }
    ...
    for(int i=0;i<nf;i++){ retro_run(); frame++; trace_row(frame); }
  }
  if(sf) fclose(sf);
