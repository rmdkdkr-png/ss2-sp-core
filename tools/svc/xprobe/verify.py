...
for name,pat in [("H stepwise",svc_H??.ram),("F P1 walks R",svc_F??.ram),("G P1 idle P2 walks",svc_G??.ram),("E P1 idle long",svc_E?.ram)]:
    d=L(pat)
    if not d: continue
    print("==",name,"n=",len(d))
    ...
    print("  hi 0935/0AB5/19A7",[x[0x0935] for x in d],[x[0x0AB5] for x in d],[x[0x19A7] for x in d])
